...
cbar.set_label(r'Spectral Radiance ($Wm^{-2}sr^{-1}$)', fontsize=20)
...
Total_IRTF_Data_Alt = np.nanmean(IRTF_Data_Q1, axis = 2)
...
plt.figure()
plt.imshow(Total_IRTF_Data_AltQ3, cmap='gist_gray', vmax = 0.02, vmin = -0.02)
plt.xlabel(r'Wavelength ' + '($\mu$m)', fontsize = 15)
#label_x = 0, 200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800, 2000
#plt.xticks(label_x, (str(wave2[0]), str(wave2[200]), str(wave2[400]), str(wave2[600]), str(wave2[800]), str(wave2[1000]), str(wave2[1200]), str(wave2[1400]), str(wave2[1600]), str(wave2[1800]), str(wave2[2000])), fontsize=10)
plt.ylabel('Spatial position across Slit (Pixel No.)', fontsize=16)
...
for x in range(11):
    ...
    if x == 8:
        for m in range(25):
            ...
            ex_A0_Q18.append(A0)
            A2 = ex_pQ1P.a2
            ex_A2_Q18.append((A2)*1.5943798828124993e-05)
            err_A0 = ex_eQ1P[0]
            ex_Err_A0_Q18.append(err_A0)
            err_A2 = ex_eQ1P[2]
            ex_Err_A2_Q18.append(err_A2*1.5943798828124993e-05)
            ex_A1_Q18.append(ex_pQ1P.a1)
            ex_Err_A1_Q18.append(ex_eQ1P[1]*1.5943798828124993e-05)
            ex_FWHM = A2*1.5943798828124993e-05*2*np.sqrt(2*math.log(2))
    ...
    for m in range(22):
        ...
        ex_A0_Q1.append(A0)
        A2 = ex_pQ1P.a2
        ex_A2_Q1.append((A2)*1.5943798828124993e-05)
        err_A0 = ex_eQ1P[0]
        ex_Err_A0_Q1.append(err_A0)
        err_A2 = ex_eQ1P[2]
        ex_Err_A2_Q1.append(err_A2*1.5943798828124993e-05)
        ex_A1_Q1.append(ex_pQ1P.a1)
        ex_Err_A1_Q1.append(ex_eQ1P[1]*1.5943798828124993e-05)
        ex_FWHM = A2*1.5943798828124993e-05*2*np.sqrt(2*math.log(2))
...
for x in range(11):
    ...
    for m in range(22):
        ...
        ex_A0_Q3.append(A0)
        A2 = ex_pQ3P.a2
        ex_A2_Q3.append((A2)*1.5709811506855093e-05)
        err_A0 = ex_eQ1P[0]
        ex_Err_A0_Q3.append(err_A0)
        err_A2 = ex_eQ1P[2]
        ex_Err_A2_Q3.append(err_A2*1.5709811506855093e-05)
        ex_A1_Q3.append(ex_pQ3P.a1)
        ex_Err_A1_Q3.append(ex_eQ3P[1]*1.5709811506855093e-05)
...
for n in range(242):
    A1 = ((ex_A1_Q1[n]-np.nanmean(ex_A1_Q1))*-1.5709811506855093e-05)
    A1_Q1.append(A1)

for n in range(242):
    A1 = ((ex_A1_Q3[n]-np.nanmean(ex_A1_Q3))*-1.5709811506855093e-05)
    A1_Q3.append(A1)
...
for n in range(22):
    A1 = ((ex_A1_Q18[n]-np.nanmean(ex_A1_Q18[3:19]))*1.5709811506855093e-05)
    A1_Q12.append(A1)
...
Vel_O4_Correct = (Velocity_Av - Planet_rotation_O4)
def lin_func(x, a , b, c): #Now to find what A0 would be for the positive and negative lines
    y = a* x**2 + b*x + c
    return y

from scipy.optimize import curve_fit

popt1, pcov1 = curve_fit(lin_func, np.arange(22), Vel_O4_Correct, p0=[0, 1, -1])
