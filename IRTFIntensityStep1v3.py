...
for x in range(11):
    ...
    if x == 8:
        for m in range(25):
            ...
            ex_resultQ1P = gmodel.fit(datai[1110:1310], x=XX, a0=0.01, a1=a1_pointP, a2=2, a3=0, a4=0, a5=0)
            ex_pQ1P = SimpleNamespace(**ex_resultQ1P.best_values)
            ex_eQ1P = np.sqrt(np.diag(ex_resultQ1P.covar))
            A0 = ex_pQ1P.a0
            ex_A0_Q18.append(A0)
            A2 = ex_pQ1P.a2
            ex_A2_Q18.append((A2)*1.6273849607182894e-05)
            err_A0 = ex_eQ1P[0]
            ex_Err_A0_Q18.append(err_A0)
            err_A2 = ex_eQ1P[2]
            ex_Err_A2_Q18.append(err_A2*1.6273849607182894e-05)
            ex_A1_Q18.append(ex_pQ1P.a1)
            ex_Err_A1_Q18.append(ex_eQ1P[1]*1.6273849607182894e-05)
            ex_FWHM = A2*1.6273849607182894e-05*2*np.sqrt(2*math.log(2))
    ...
    for m in range(22):
        ...
        ex_A0_Q1.append(A0)
        A2 = ex_pQ1P.a2
        ex_A2_Q1.append((A2)*1.6273849607182894e-05)
        err_A0 = ex_eQ1P[0]
        ex_Err_A0_Q1.append(err_A0)
        err_A2 = ex_eQ1P[2]
        ex_Err_A2_Q1.append(err_A2*1.6273849607182894e-05)
        ex_A1_Q1.append(ex_pQ1P.a1)
        ex_Err_A1_Q1.append(ex_eQ1P[1]*1.6273849607182894e-05)
        ex_FWHM = A2*1.6273849607182894e-05*2*np.sqrt(2*math.log(2))
        ...
        #         z = (XX - ex_pQ1P.a1) / ex_pQ1P.a2
        #         y = ex_pQ1P.a0 * np.exp(-z**2 / ex_pQ1P.a2) + ex_pQ1P.a3 + ex_pQ1P.a4 * XX + ex_pQ1P.a5 * XX**2
        #         plt.plot(XX, y, 'g')
...
for x in range(11):
    ...
    for m in range(21):
        ...
        ex_A0_Q3.append(A0)
        A2 = ex_pQ3P.a2
        ex_A2_Q3.append((A2)*1.6361537623956223e-05)
        err_A0 = ex_eQ1P[0]
        ex_Err_A0_Q3.append(err_A0)
        err_A2 = ex_eQ1P[2]
        ex_Err_A2_Q3.append(err_A2*1.6361537623956223e-05)
        ex_A1_Q3.append(ex_pQ3P.a1)
        ex_Err_A1_Q3.append(ex_eQ3P[1]*1.6361537623956223e-05)
        ex_FWHM = A2*1.6361537623956223e-05*2*np.sqrt(2*math.log(2))
...
Mapping_Q3A = np.reshape(ex_INTSQ3, (11, 21))
...
Mapping_Q3Err = np.reshape(Q3_IntErr, (11, 21))
...
# for a in range(11):
#     data = (IRTF_Data_Q1[:,:,b] + IRTF_Data_Q1[:,:,b+1] + IRTF_Data_Q1[:,:,b+2] + IRTF_Data_Q1[:,:,b+3])/8
#     data[data > 0.1] = 0
#     data[data < -0.1] = 0
...
#             dataii = data[ns+m, :]
#             gmodel = Model(gauss_fit)
#             a1_pointP = np.where(datai == np.nanmax(datai[1210:1220]))
#             a1_pointP = a1_pointP[0][0] - 1110
#             a1_pointN = np.where(dataii == np.nanmin(dataii[1210:1220]))
#             a1_pointN = a1_pointN[0][0] - 1110
#             ex_resultQ1P = gmodel.fit(datai[1110:1310], x=XX, a0=np.nanmax(datai[1195:1225]), a1=a1_pointP, a2=2, a3=0, a4=0, a5=0)
#             ex_resultQ1N = gmodel.fit(dataii[1110:1310], x=XX, a0=np.nanmin(dataii[1195:1225]), a1=a1_pointN, a2=2, a3=0, a4=0, a5=0)
#             ex_pQ1P = SimpleNamespace(**ex_resultQ1P.best_values)
#             ex_eQ1P = np.sqrt(np.diag(ex_resultQ1P.covar))
#             ex_pQ1N = SimpleNamespace(**ex_resultQ1N.best_values)
#             ex_eQ1N = np.sqrt(np.diag(ex_resultQ1N.covar))
#             A0 = (ex_pQ1P.a0 - ex_pQ1N.a0)/2
#             ex_A0_Q1.append(A0)
#             A1 = (ex_pQ1P.a1 + ex_pQ1N.a1)/2
#             ex_A1_Q1.append(A1)
#             A2 = (ex_pQ1P.a2+ex_pQ1N.a2)/2
#             ex_A2_Q1.append((A2)*1.6273849607182894e-05)
#             err_A0 = (ex_eQ1P[0] + ex_eQ1N[0])/2
#             ex_Err_A0_Q1.append(err_A0)
#             err_A1 = (ex_eQ1P[1] + ex_eQ1N[1])/2
#             ex_Err_A1_Q1.append(err_A1*1.6273849607182894e-05)
#             err_A2 = (ex_eQ1P[2]+ex_eQ1N[2])/2
#             ex_Err_A2_Q1.append(err_A2*1.6273849607182894e-05)
#             ex_FWHM = A2*1.6273849607182894e-05*2*np.sqrt(2*math.log(2))
#             ex_FWHMQ1.append(ex_FWHM)
#             ex_INTSQ1.append(A0*ex_FWHM*(10**6))
...
# Mapping_Q1A = Mapping_Q1A.transpose()
# Mapping_Q1A = np.flipud(Mapping_Q1A)
# #Mapping_Q1B = np.where(Mapping_Q1A==Mapping_Q1A[8,0], np.nanmean(Mapping_Q1A[4:8,0]), Mapping_Q1A)
# plt.figure()
# plt.imshow(np.fliplr(Mapping_Q1A), cmap='nipy_spectral')
# cbar = plt.colorbar()
# cbar.set_label(r'Intensity (${\mu}Wm^{-2}sr^{-1}$) +- ' + str("{:.2f}".format(round(np.nanmean(Q1_IntErr[75:154]), 2))) + ' ${\mu}Wm^{-2}sr^{-1}$', fontsize=17.5) 
# plt.title(r'$H_{3}^{+}$ $Q(1,0^{-})$ Intensity of Uranus observations from 07:30 to 12:57 UTC on $11^{th}$ October 2016', pad = 45, fontsize = 20)
# plt.xlabel('Arbitrary Longitude across Uranus (Pixels)', fontsize=17.5, labelpad=10)
...
# # plt.ylabel('UTC Time (HH:MM +- 00:15)', fontsize=17.5, labelpad=10)
# # #plt.yticks(np.arange(11), TIMEv2)
# #%% Time to do the same thing with Q3
# #Need to check if we can do the same numbers of sets as Q1
# b = 0
...
# for a in range(5):
#     data = (IRTF_Data_Q3[:,:,b] + IRTF_Data_Q3[:,:,b+1] + IRTF_Data_Q3[:,:,b+2] + IRTF_Data_Q3[:,:,b+3] + IRTF_Data_Q3[:,:,b+4] + IRTF_Data_Q3[:,:,b+5] + IRTF_Data_Q3[:,:,b+6] + IRTF_Data_Q3[:,:,b+7])/16
#     data[data > 0.1] = 0
#     data[data < -0.1] = 0
#     data = acre(data, width = 5, verbose = False)
#     b += 8
...
#             ex_A1_Q3.append(A1)
#             A2 = 0
#             ex_A2_Q3.append((A2)*1.6273849607182894e-05)
#             err_A0 = 0
#             ex_Err_A0_Q3.append(err_A0)
#             err_A2 = 0
#             ex_Err_A2_Q3.append(err_A2*1.6273849607182894e-05)
#             err_A1 = 0
#             ex_Err_A1_Q3.append(err_A1)
#             ex_FWHM = A2*1.6273849607182894e-05*2*np.sqrt(2*math.log(2))
#             ex_FWHMQ3.append(ex_FWHM)
#             ex_INTSQ3.append(A0*ex_FWHM*(10**6))
#         else:
#             datai = data[ps+m, :]
#             datailoc = np.where(datai == np.max(datai[1355:1365]))
#             datailoc = datailoc[0][0] - 1255
#             dataPi = datai
#             dataii = data[ns+m, :]
#             dataiiloc = np.where(dataii == np.min(dataii[1355:1365]))
#             dataiiloc = dataiiloc[0][0] - 1255
#             dataNi = dataii
#             gmodel = Model(gauss_fit)
#             ex_resultQ3P = gmodel.fit(dataPi[1255:1455], x=XX, a0=np.nanmax(dataPi[1355:1365]), a1=datailoc, a2=2, a3=0, a4=0, a5=0)
#             ex_resultQ3N = gmodel.fit(dataNi[1255:1455], x=XX, a0=np.nanmin(dataNi[1355:1365]), a1=dataiiloc, a2=2, a3=0, a4=0, a5=0)
#             ex_pQ3P = SimpleNamespace(**ex_resultQ3P.best_values)
#             ex_eQ3P = np.sqrt(np.diag(ex_resultQ3P.covar))
#             ex_pQ3N = SimpleNamespace(**ex_resultQ3N.best_values)
#             ex_eQ3N = np.sqrt(np.diag(ex_resultQ3N.covar))
#             A0 = (ex_pQ3P.a0 - ex_pQ3N.a0)/2
#             ex_A0_Q3.append(A0)
#             A1 = (ex_pQ3P.a1 + ex_pQ3N.a1)/2
#             ex_A1_Q3.append(A1)
#             A2 = (ex_pQ3P.a2+ex_pQ3N.a2)/2
#             ex_A2_Q3.append((A2)*1.6273849607182894e-05)
#             err_A0 = (ex_eQ3P[0] + ex_eQ3N[0])/2
#             ex_Err_A0_Q3.append(err_A0)
#             err_A1 = (ex_eQ3P[1] + ex_eQ3N[1])/2
#             ex_Err_A1_Q3.append(err_A1*1.6273849607182894e-05)
#             err_A2 = (ex_eQ3P[2]+ex_eQ3N[2])/2
#             ex_Err_A2_Q3.append(err_A2*1.6273849607182894e-05)
#             ex_FWHM = A2*1.6273849607182894e-05*2*np.sqrt(2*math.log(2))
#             ex_FWHMQ3.append(ex_FWHM)
#             ex_INTSQ3.append(A0*ex_FWHM*(10**6))
 
# # #Now to plot all five lines
# Mapping_Q3A = np.reshape(ex_INTSQ3, (5, 22))
# Mapping_Q3A = Mapping_Q3A.transpose()
# Mapping_Q3A[Mapping_Q3A == 0] = np.nan
# #  
# for o in range(110):
...
# #%% Q3 maybe a bust so lets just try to place Q1 across a sphere
# TIME = ['07:59', '08:59', '09:58', '10:58', '11:57']
# TIMEv2 = ['11:57', '10:58', '09:58', '08:59', '07:59']

# Mapping_Q3B = Mapping_Q3A.transpose()
# Mapping_Q3B = np.flipud(Mapping_Q3B)
# plt.figure()
# plt.imshow(np.fliplr(Mapping_Q3B), cmap='nipy_spectral')
# cbar = plt.colorbar()
# cbar.set_label(r'Intensity (${\mu}Wm^{-2}sr^{-1}$) +- ' + str("{:.2f}".format(round(np.nanmean(Q3_IntErr[75:154]), 2))) + ' ${\mu}Wm^{-2}sr^{-1}$', fontsize=17.5) 
# plt.title(r'$H_{3}^{+}$ $Q(3,0^{-})$ Intensity of Uranus observations from 07:30 to 12:57 UTC on $11^{th}$ October 2016', pad = 45, fontsize = 20)
# plt.xlabel('Arbitrary Longitude across Uranus (Pixels)', fontsize=17.5, labelpad=10)
# plt.ylabel('UTC Time (HH:MM +- 00:30)', fontsize=17.5, labelpad=10)
# plt.yticks(np.arange(5), TIMEv2)
...
# Temps_Err = Temperature_Err/(math.log(2.718281828459)*(np.nanmean(Mapping_Q1A)/np.nanmean(Mapping_Q3B)))
...
# Imodel_Q1 = (gQ1 * JQ1  *wQ1 * AQ1 * h * c * 100)/(4*np.pi) #etc
...
for n in range(242):
    A1 = ((ex_A1_Q1[n]-np.nanmean(ex_A1_Q1))*1.6273849607182894e-05)
    A1_Q1.append(A1)

for n in range(231):
    A1 = ((ex_A1_Q3[n]-np.nanmean(ex_A1_Q3))*1.6361537623956223e-05)
    A1_Q3.append(A1)
    
for n in range(25):
    A1 = ((ex_A1_Q18[n]-np.nanmean(ex_A1_Q18))*1.6361537623956223e-05)
    A1_Q18.append(A1)
    
    
Q1_line_shift = np.reshape(A1_Q1, (11, 22))
Q3_line_shift = np.reshape(A1_Q3, (11, 21))
...
Period_Uranus = Circumference_Uranus/Time_s_Uranus
...
Limb_velocity_pix_O4 = Period_Uranus/Uranus_width_pix_O4
Limb_velocity_pix_O5 = Period_Uranus/Uranus_width_pix_O5
...
for n in range(242):
    V = ((A1_Q1[n]/lamda_H3_O4)*c)/Uranus_width_pix_O4
    Velocity_O4.append(V)

for n in range(25):
    V = ((A1_Q18[n]/lamda_H3_O4)*c)/Uranus_width_pix_O4
    Velocity_O48.append(V)

for n in range(231):
    V = ((A1_Q3[n]/lamda_H3_O5)*c)/Uranus_width_pix_O5  
    Velocity_O5.append(V)
...
for n in range(242):
    Err = Velocity_O4[n]*np.sqrt((Err1/Uranus_width_pix_O4)**2+(ex_Err_A1_Q1[n]/A1_Q1[n])**2)
    Velocity_O4_Err.append(np.sqrt(Err**2))

for n in range(25):
    Err = Velocity_O48[n]*np.sqrt((Err1/Uranus_width_pix_O4)**2+(ex_Err_A1_Q18[n]/A1_Q18[n])**2)
    Velocity_O48_Err.append(np.sqrt(Err**2))

for n in range(231):
    Err = Velocity_O5[n]*np.sqrt((Err3/Uranus_width_pix_O5)**2+(ex_Err_A1_Q3[n]/A1_Q3[n])**2)
    Velocity_O5_Err.append(np.sqrt(Err**2))
    
Velocities_O4 = np.reshape(Velocity_O4, (11, 22))
Velocities_O4 = Velocities_O4.transpose()

Velocity_Err_O4 = np.reshape(Velocity_O4_Err, (11, 22))
Velocity_Err_O4 = Velocity_Err_O4.transpose()
...
Planet_rotation_O48 = np.linspace(Limb_velocity_pix_O4, -1*Limb_velocity_pix_O4, 25)
...
for a in range(11):
    ...
    ax2 = ax.twinx()
    ax.set_title('Approximate ion velocities from $H_{3}^{+}$ $Q(1,0^{-})$ over a ~30 minute exposure at ' +str(TIME[10-k]) +' UT', fontsize=25)
    ax.plot(np.arange(22), np.flip(Planet_rotation_O4), color='k', label='Planetary Rotation')
    ax.plot(np.arange(22), np.flip(Velocities_O4[:,k]), color='b', label='IR Velocities', lw = 3)
    ax.fill_between(np.arange(22), np.flip(Velocities_O4[:,k])-np.flip(Velocity_Err_O4[:,k]), np.flip(Velocities_O4[:,k])+np.flip(Velocity_Err_O4[:,k]), color='b', alpha=0.5)
    ...
    ax.set_ylabel('Wind Speeds from $H_{3}^{+}$ $Q(1,0^{-})$ across Uranus (ULS) ($^\circ$)', fontsize=20, labelpad=15)
    ax2.set_ylabel(r'Intensity (${\mu}Wm^{-2}sr^{-1}$) +- ' + str("{:.2f}".format(round(np.nanmean(Q1_IntErr), 2))) + ' ${\mu}Wm^{-2}sr^{-1}$', fontsize=20)
...
fig, ax = plt.subplots(figsize=(12,8))
ax2 = ax.twinx()
ax.set_title('Approximate ion velocities from $H_{3}^{+}$ $Q(1,0^{-})$ over a ~30 minute exposure at ' +str(TIME[9-k]) +' UT', fontsize=25)
ax.plot(np.arange(25), np.zeros(25), color='k', label='Planetary Rotation')
ax.plot(np.arange(25), np.flip(Velocity_O48 - Planet_rotation_O48), color='b', label='IR Velocities', lw = 3)
ax.fill_between(np.arange(25), np.flip(Velocity_O48 - Planet_rotation_O48)-np.flip(Velocity_O48_Err), np.flip(Velocity_O48 - Planet_rotation_O48)+np.flip(Velocity_O48_Err), color='b', alpha=0.5)
ax2.plot(np.arange(25), np.flip(ex_INTSQ18), color='r', label='IR Intensity', lw = 3)
#ax.xticks(np.arange(0, 181, 30), ['0$^\circ$', '20$^\circ$', '40$^\circ$', '60$^\circ$', '80$^\circ$', '100$^\circ$', '120$^\circ$', '140$^\circ$', '160$^\circ$', '180$^\circ$', '200$^\circ$', '220$^\circ$', '240$^\circ$', '260$^\circ$', '280$^\circ$', '300$^\circ$', '320$^\circ$', '340$^\circ$', '360$^\circ$'], fontsize=15)
# #plt.xticks(np.arange(0, 181, 30), ['0$^\circ$', '30$^\circ$', '60$^\circ$', '90$^\circ$', '120$^\circ$', '150$^\circ$', '180$^\circ$'], fontsize=15)
# #plt.xticks(np.arange(0, 361, 30), ['0$^\circ$', '30$^\circ$', '60$^\circ$', '90$^\circ$', '120$^\circ$', '150$^\circ$', '180$^\circ$', '210$^\circ$', '240$^\circ$', '270$^\circ$', '300$^\circ$', '330$^\circ$', '360$^\circ$'], fontsize=15)
...
ax.set_ylabel('Wind Speeds from $H_{3}^{+}$ $Q(1,0^{-})$ across Uranus (ULS) ($^\circ$)', fontsize=20, labelpad=15)
ax2.set_ylabel(r'Intensity (${\mu}Wm^{-2}sr^{-1}$) +- ' + str("{:.2f}".format(round(np.nanmean(Q1_IntErr), 2))) + ' ${\mu}Wm^{-2}sr^{-1}$', fontsize=20)
ax.vlines(np.linspace(0, 24, 7), -50, 50, colors = 'k', linestyles = 'dotted', alpha = 0.75)
ax.hlines(np.linspace(-150, 150, 7), 0, 24, colors = 'k', linestyles = 'dotted', alpha = 0.75)
ax.tick_params(axis='both', labelsize=15)
ax2.tick_params(axis='both', labelsize=15)
ax.set_xlim(0, 24) #0, 3601
ax.set_ylim(-50, 50)
ax2.set_ylim(0, 1.0)
ax.legend(fontsize=15)
ax2.legend(loc='upper left', fontsize=15)

LongitudesS = np.load('LongitudesS.npy')
Mean_Longs = []

for a in range(18):
    if a == 0 or a == 17:
        Mean_Longitude = (LongitudesS[a] + -1*LongitudesS[a+18])/2
    else:
        Mean_Longitude = (LongitudesS[a] + LongitudesS[a+18])/2
    Mean_Longs.append(int(Mean_Longitude))

fig, ax = plt.subplots(figsize=(12,8))
ax2 = ax.twinx()
ax.set_title('Approximate ion velocities from $H_{3}^{+}$ $Q(1,0^{-})$ over a ~30 minute exposure at ' +str(TIME[9-k]) +' UT', fontsize=25)
ax.plot(Mean_Longs, np.flip(Planet_rotation_O48[4:-3]), color='k', label='Planetary Rotation')
ax.plot(Mean_Longs, np.flip(Velocity_O48[4:-3]), color='b', label='IR Velocities', lw = 3)
ax.fill_between(Mean_Longs, np.flip(Velocity_O48[4:-3])-np.flip(Velocity_O48_Err[4:-3]), np.flip(Velocity_O48[4:-3])+np.flip(Velocity_O48_Err[4:-3]), color='b', alpha=0.5)
#ax2.plot(np.arange(25), np.flip(ex_INTSQ18), color='r', label='IR Intensity', lw = 3)
#ax.xticks(np.arange(0, 181, 30), ['0$^\circ$', '20$^\circ$', '40$^\circ$', '60$^\circ$', '80$^\circ$', '100$^\circ$', '120$^\circ$', '140$^\circ$', '160$^\circ$', '180$^\circ$', '200$^\circ$', '220$^\circ$', '240$^\circ$', '260$^\circ$', '280$^\circ$', '300$^\circ$', '320$^\circ$', '340$^\circ$', '360$^\circ$'], fontsize=15)
# #plt.xticks(np.arange(0, 181, 30), ['0$^\circ$', '30$^\circ$', '60$^\circ$', '90$^\circ$', '120$^\circ$', '150$^\circ$', '180$^\circ$'], fontsize=15)
# #plt.xticks(np.arange(0, 361, 30), ['0$^\circ$', '30$^\circ$', '60$^\circ$', '90$^\circ$', '120$^\circ$', '150$^\circ$', '180$^\circ$', '210$^\circ$', '240$^\circ$', '270$^\circ$', '300$^\circ$', '330$^\circ$', '360$^\circ$'], fontsize=15)
ax.set_xticks(np.linspace(-90, 90, 7))
#ax.set_xticklabels(labels=Mean_Longs)
# #plt.vlines((0, 200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800, 2000, 2200, 2400, 2600, 2800, 3000, 3200, 3400, 3600), 0, 3600, colors = 'k', linestyles = 'dotted', alpha = 0.4)
ax.set_xlabel('Arbitrary Uranian Longitude ($^\circ$)', fontsize=20, labelpad=15)
ax.set_ylabel('Ion velocities from $H_{3}^{+}$ $Q(1,0^{-})$ across Uranus (ULS) ($^\circ$)', fontsize=20, labelpad=15)
#ax2.set_ylabel(r'Intensity (${\mu}Wm^{-2}sr^{-1}$) +- ' + str("{:.2f}".format(round(np.nanmean(Q1_IntErr), 2))) + ' ${\mu}Wm^{-2}sr^{-1}$', fontsize=20)
ax.vlines(np.linspace(-90, 90, 7), -150, 150, colors = 'k', linestyles = 'dotted', alpha = 0.75)
ax.hlines(np.linspace(-150, 150, 7), -90, 90, colors = 'k', linestyles = 'dotted', alpha = 0.75)
ax.tick_params(axis='both', labelsize=15)
ax2.tick_params(axis='both', labelsize=15)
...
#%%
Velocities_O5 = np.reshape(Velocity_O5, (11, 21))
Velocities_O5 = Velocities_O5.transpose()

Velocity_Err_O5 = np.reshape(Velocity_O5_Err, (11, 21))
...
#%%Add all lines #NEED TO CHECK FROM HERE ONWARDS!!!!!
#For Q1
...
# plt.figure()
# plt.imshow(Test_Total, cmap='gist_gray', vmax = np.nanmean(Test_1)+np.nanstd(Test_1), vmin = np.nanmean(Test_1)-np.nanstd(Test_1))
...
# for n in range(484):
#     V = ((A1_T[n]/lamda_H3)*c)*2/(Uranus_width_pix_O4+Uranus_width_pix_O5)
#     Velocity_T.append(V)
...
# for n in range(484):
#     Err = Velocity_T[n]*np.sqrt((ErrorA)**2+(Err_A1_loc_T[n]/A1_loc_T[n])**2)
#     Velocity_Err.append(np.sqrt(Err**2))
